...
#### Poller模块
1. 功能: 描述符IO监控模块
2. 设计思想: 通过epoll实现描述符的IO监控(通过对epoll的封装)
3. 要素:
- 增加/修改文件描述符的监控
- 移除对文件描述符的监控
...
#### TimeWheel模块
定时器模块整合
1. timefd实现内核每隔一段时间给进程一次超时事件
2. timeerwheel实现每次执行runtimetask，都可以执行一次到期的定时任务

实现一个完整的秒级定时器，需要将两个模块整合到一起，当事件被触发，直接到期的定时任务

#### Connection模块
目的: 对连接进行全方位管理，对链接的所有操作都是通过这个模块完成
功能设计: 
1. 套接字管理，能够进行套接字操作
2. 链接事件的管理，可读，可写，错误，挂断，任意
3. 缓冲区管理
4. 协议上下文管理
