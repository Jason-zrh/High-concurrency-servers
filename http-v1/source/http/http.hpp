...
private:
    void ErrorHandler(const HttpRequest &req, HttpResponse *rsp)
    {
        ...
        body += " ";
        body += Util::StatuDescribe(rsp->_statu);
        body += "</h1>";
        body += "</body>";
        body += "</html>";
    ...
    void WriteReponse(const ConnectionPtr &conn, const HttpRequest &req, HttpResponse &rsp)
    {
        // 1. 先完善头部字段
        if (req.Close() == true)
        {
        ...
        rsp_str << req._version << " " << std::to_string(rsp._statu) << " " << Util::StatuDescribe(rsp._statu) << "\r\n";
...
public:
    ...
    : _server(port)
    {
        _server.EnableInactiveRealse(timeout);
        _server.SetConnectCallBack(std::bind(&HttpServer::OnConnected, this, std::placeholders::_1));
        _server.SetMsgCallBack(std::bind(&HttpServer::OnMessage, this, std::placeholders::_1, std::placeholders::_2));
    }

    void SetBaseDir(const std::string &path)
    {
        assert(Util::IsDirection(path) == true);
        _basedir = path;
    }
