...
inline static void* SystemAlloc(size_t kpage)
{
    ...
    void* raw = mmap(nullptr, bytes + align, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    if (raw == MAP_FAILED)
    {
        ptr = nullptr;
    }
    else
    {
        ...
        size_t prefix = alignedAddr - rawAddr;
        size_t suffix = align - prefix;
        if (prefix != 0)
        {
            munmap(raw, prefix);
        }
        if (suffix != 0)
        {
            munmap(reinterpret_cast<void*>(alignedAddr + bytes), suffix);
        }
